...
namespace progtracklabel {
	...
	namespace reset_layer {
		void set_enabled(progtracklabel::LabelLayer* layer_) {
			layer_->setVisible(Mod::get()->getSettingValue<bool>("enable-mod"));
		}
	...
	protected:
		...
		bool init() {
			...
			auto menu_container = this->getChildByIndex(0)->getChildByType<CCMenu>(0);
...
$execute {
	...
	if (true) {  // Enable Setting Change tracking
		listenForSettingChanges<bool>("enable-mod", [](bool) {
			auto layer_ = (progtracklabel::LabelLayer*) CCScene::get()->getChildByIDRecursive("progresstracklabel-labellayer");
			if (layer_ != nullptr)
				progtracklabel::reset_layer::set_enabled(layer_);
		});
		...
		for (int i = 1; i < 10; i++)
			this->addEventListener(
				...
				log::debug("Button {} // keybind {}, down {}, repeat {}, timestamp {}", i, keybind.toString(), down, repeat, timestamp);
					...
					&& Mod::get()->getSavedValue<bool>(fmt::format("counter-enablebind-{}", i))
					&& (Mod::get()->getSavedValue<bool>(fmt::format("counter-available-{}", i))
						...
						if (play_layer != nullptr && !play_layer->m_isPaused) {
...
class $modify(PauseLayerModified, PauseLayer) {
	...
	void openMenu(CCObject*) {
		...
		progtracklabel::PLTLayer* menu_layer = progtracklabel::PLTLayer::create(play_layer->m_level);
